...
short_hmm_colors <- c('Art', 'Ctcf', 'CtcfO', 'PromP', 'PromF', 'Tss')
for (i in 1:length(short_hmm_colors)) {
    for (j in i+1:length(short_hmm_colors)) {
        ...
        foo <- cramer.test(head(as.matrix(d[d[,hmm_colors[i]],c('beta', 'standardized_entropy')]),
                                DOWNSAMPLING),
                           head(as.matrix(d[d[,hmm_colors[j]],c('beta', 'standardized_entropy')]),
                                DOWNSAMPLING),
                           sim = "eigenvalue")
