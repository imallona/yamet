...
scmet_epsilon_meth <- ggplot(jnt, aes(x = avg_meth, y = scmet_epsilon, color = bc_vi)) +
  ...
  labs(
    ...
    x = "avg. methylation", y = expression("scMET " ~ epislon), color = "acVI"
  )
...
# larger axis labels for assembled figure (>=10 pt when rendered)
big_labels <- theme(axis.title = element_text(size = 14), axis.text = element_text(size = 11))
samp_meth          <- samp_meth + big_labels
samp_shan          <- samp_shan + big_labels
scmet_mu_meth      <- scmet_mu_meth + big_labels
scmet_gamma_meth   <- scmet_gamma_meth + big_labels
scmet_epsilon_meth <- scmet_epsilon_meth + big_labels
...
arranged <- ggarrange(
  ...
  ggarrange(
    ...
    ggarrange(
      s_plots$nmi   + theme(legend.position = "none") + xlab('# cells') + ylab("performance (metric NMI)") + big_labels,
      s_plots$seconds  + theme(legend.position = "none") + xlab('# cells') + ylab("CPU time (s)") + big_labels,
      s_plots$mem + theme(legend.position = "none") + xlab('# cells') + ylab("memory usage (MB)") + big_labels,
      n_plots$nmi   + theme(legend.position = "none") + xlab('# features')+ ylab("performance (metric NMI)") + big_labels,
      n_plots$seconds  + theme(legend.position = "none") + xlab('# features') + ylab("CPU time (s)") + big_labels,
      n_plots$mem   + theme(legend.position = "none") + xlab('# features') + ylab("memory usage (MB)") + big_labels,
      ncol = 3,
      nrow = 2
    ),
...
ggsave(
  filename = "output/figure_atreya_simulations_across.svg",
  plot   = arranged,
  width  = 15,
  height  = 15,
  units   = "in"
