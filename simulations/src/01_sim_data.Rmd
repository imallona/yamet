...
```{r, paths}
lowRealDir <- "./data/low_real/met/cpg_level"
...
```{r, required external data, eval=FALSE}
# rather read from config
if("low_real" %in% paramGrid$covParams){
...
## Simulate
```{r, simulate nb, eval=TRUE, warning=FALSE,eval=FALSE}
for(i in 1:nrow(paramGrid)){
  ...
  else if(paramsSet$transMat=="imr_cons"){
    transMat <- matrix(c(0.8,0.2,0.2,0.8), nrow=2, ncol=2)
    estimateTransMat=FALSE
  }
  else if(paramsSet$transMat=="imr_rand"){
