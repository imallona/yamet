...
main_plots$samp_meth <- ggplot(all.long, aes(x = meth, y = sampen, color = wc_vi)) +
...
main_plots$samp_shan <- ggplot(all.long, aes(x = shannon, y = sampen, color = wc_vi)) +
  geom_point(alpha = 0.7) +
  theme_ng() +
  labs(
    ## title = "Sample Entropy in different wcVI values",
...
main_plots$scmet_mu_meth <- ggplot(jnt, aes(x = avg_meth, y = scmet_mu, color = wc_vi)) +
  geom_point(alpha = 0.7) +
  theme_ng() +
  labs(
...
main_plots$scmet_gamma_meth <- ggplot(jnt, aes(x = avg_meth, y = scmet_gamma, color = wc_vi)) +
  geom_point(alpha = 0.7) +
  theme_ng() +
  labs(
...
main_plots$scmet_epsilon_meth <- ggplot(jnt, aes(x = avg_meth, y = scmet_epsilon, color = wc_vi)) +
  geom_point(alpha = 0.7) +
  theme_ng() +
  labs(
...
saveRDS(mi_fd, paste0(dirname(snakemake@output[["html"]]), "/05_perf.rds"))
```
...
legends <- list()
legends$wcVI <- get_legend(main_plots$samp_meth)
...
saveRDS(list(
  ...
  dirname(snakemake@output[["html"]]), "/05_plots.rds"
))
...
ggsave(
  filename = paste0(
    dirname(snakemake@output[["html"]]), "/05_within.svg"
  ),
  plot = arranged,
  width = 15,
  height = 15,
