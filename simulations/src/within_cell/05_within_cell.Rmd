...
make_grid_plots <- function(GRID, grid_var) {
  ...
  p <- ggplot(mi_data, aes(x = .data[[grid_var]], y = NMI, color = Metric, group = Metric)) +
    scale_y_log10() +
  ...
  p <- ggplot(bm_data, aes(x = .data[[grid_var]], y = s, color = mtd, group = mtd)) +
    scale_y_log10() +
  ...
  p <- ggplot(bm_data, aes(x = .data[[grid_var]], y = max_rss, color = mtd, group = mtd)) +
    scale_y_log10() +
    geom_point(size = 3) +
    stat_summary(fun = mean, geom = "line", linewidth = 1.2, aes(group = mtd)) +
    scale_color_manual(values = c("yamet" = "#E69F00", "scmet" = "#56B4E9")) +
