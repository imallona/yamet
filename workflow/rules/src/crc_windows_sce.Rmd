...
ggplot(df_crc01, aes(x = location, y = mean_sampen_corrected, fill = factor(bin))) +
  ...
  facet_grid( ~ feature) +
  labs(x = "Feature present", y = "corrected sampen (avg per cell)") +
...
ggplot(df_crc01, aes(x = location,
  ...
  geom_violin(trim = FALSE, position = position_dodge(width = 0.8), alpha = 0.6) +
    geom_jitter(aes(color = factor(bin)),
                ...
                size = 0.5, alpha = 0.5) +
  facet_grid(~ feature) +
  labs(x = "Feature present",
       y = "Corrected sampen (avg per cell)",
       fill = "Bin", color = "Bin") +
  theme_bw(base_size = 8)
```

Repeat for the diff entropic features
...
Again why are some contrasts fully missing any point? shall we skip them from plotting? Let's get some basic stats about numbers of up/down, per comparison
