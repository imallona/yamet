...
# now recompute variance explained
vars <- c(
    "location",
    "patient",
    grep("_bin_median$", colnames(colData(sce)), value = TRUE)
)


ve <- getVarianceExplained(
    sce,
    exprs_values = "sampen_corrected",
    variables = vars
)
...
ve_table <- data.frame(
    variable = names(ve_vec),
    variance_explained = as.numeric(ve_vec),
    stringsAsFactors = FALSE
)

ve_table <- ve_table[order(ve_table$variance_explained, decreasing = TRUE), ]

## print(ve_table)
```

```{r, fig.height = 15, fig.width = 10}
ggplot(ve_table, aes(x = reorder(variable, variance_explained),
                     y = variance_explained)) +
    geom_col() +
    coord_flip() +
    labs(x = "Variable", y = "Mean variance explained") +
    theme_ng()
...
this was absurd, they're not orthogonal, of course variance explained is low

todo add the up and down diff entropy as well
...
# location, but also annotations!
predictors <- c("patient", "location",
                grep("_bin_median$", colnames(cd), value = TRUE))

updown <- grep("up|down", colnames(cd), ignore.case = TRUE, value = TRUE)
predictors <- c(predictors, updown)

## no scna, multiple patients here
...
matplot(ann_unexpected, type = "l", lty = 1,
        main = "Unexpected entropy per annotation",
        ylab = "Residual SampEn")
