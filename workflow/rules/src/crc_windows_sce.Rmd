...
```{r}
windows_sce <- readRDS(snakemake@input[['windows_sce']])
...
df <- reshape2::melt(as.matrix(assay(windows_sce, "sampen_corrected")[,1:200])) # subset for plotting
...
## windows_sce <- runPCA(windows_sce, exprs_values="sampen_corrected")
## plotReducedDim(windows_sce, "PCA", colour_by="location")

## ve <- getVarianceExplained(windows_sce,
...
Row annotation (rowData) is re-read from the annotation file here because the SCE was built
from a partial read in crc_windows.Rmd; the full annotation is loaded and assigned below.


```{r}
## str(rowData(windows_sce))
table(rowData(windows_sce)$`11_Promoter_hmm`, useNA="ifany")
...
rowData(windows_sce) <- annot[rownames(windows_sce),]

summary(rowData(windows_sce)$`X11_Promoter_hmm`, useNA="ifany")
```

Binarize the annotations: any non-zero fractional overlap becomes 1.


```{r}
rd <- as.data.frame(rowData(windows_sce))
...
rownames(rd_bin) <- rownames(rowData(windows_sce))
rowData(windows_sce) <- cbind(rowData(windows_sce), rd_bin)

saveRDS(windows_sce, snakemake@params[['corrected_sce']])

summary(rowData(windows_sce)$`X11_Promoter_hmm`, useNA="ifany")
table(rowData(windows_sce)$`X11_Promoter_hmm_bin`, useNA="ifany")
...
for (contrast in names(de)) {
    ...
    rowData(windows_sce)[,paste0(contrast, '_up_bin')] <- rownames(windows_sce) %in% up
    rowData(windows_sce)[,paste0(contrast, '_down_bin')] <- rownames(windows_sce) %in% down
}
```
...
Differential entropy was tested across all patients jointly; per-patient visualizations
show the location effect within a single patient.

Association of diff entropic windows with genomic features:
...
prop_df <- prop_df %>%
  mutate(
    direction    = ifelse(grepl("_up_", contrast), "Up", "Down"),
    contrast_clean = gsub("_(up|down)_bin", "", contrast),
    annotation   = gsub("_bin$", "", annotation)
  ) %>%
...
ggplot(prop_df, aes(x = contrast_clean, y = annotation)) +
  ...
  labs(x = "contrast", y = "annotation", size = "proportion", color = "direction",
       title = "annotation overlap in diff. entropy windows (non-HMM)") +
  theme_ng() +
...
Same analysis restricted to chromatin state (HMM) annotations:
...
prop_df <- prop_df %>%
  mutate(annotation = gsub("_bin$", "", annotation)) %>%
  filter(!is.na(proportion))
...
ggplot(prop_df, aes(x = contrast_clean, y = annotation)) +
  ...
  facet_wrap(~ direction, nrow = 1) +
  labs(x = "contrast", y = "annotation", size = "proportion", color = "direction",
       title = "annotation overlap in diff. entropy windows (HMM)") +
  theme_ng() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.text = element_text(size = 7))
...
expr <- assay(windows_sce, "sampen_corrected")

rd <- as.data.frame(rowData(windows_sce))[,grep('_bin$', colnames(rowData(windows_sce)))]
str(rd)
...
for (j in seq_len(ncol(rd_mat))) {
    ...
    cell_ann_median[, j] <- apply(subexpr, 2, median, na.rm = TRUE)
}
...
Per-cell entropy unexplained by annotation-level predictors, fit on CRC01 only.
...
keep_pred <- rowSums(is.na(cor_mat)) < nrow(cor_mat)
cor_mat_clean <- cor_mat[keep_pred, keep_pred]
...
boxplot(cell_unexpected ~ cd$location,
        main = "Residual entropy per biopsy location",
