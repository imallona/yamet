...
full_annot <- read_tsv(snakemake@input[["annotations"]], show_col_types = FALSE)
...
for (contrast in names(de)) {
  ...
  tmp <- merge(full_annot, coef_tab[, c("region","moderated_t")],
               by = "region", all.x = TRUE)
  full_annot[[contrast]] <- tmp$moderated_t
  rm(coef_tab)
}


str(full_annot)
summary(full_annot$ln_vs_nc)
```
