...
# Same but with a function, PT vs NC
...
head(res2$coefs_df)
hist(res2$coefs_df$p_val)
table(res2$coefs_df$adj_p < 0.05)
```

# Same, LN vs NC

```{r}
res3 <- diff_entropy_test(
  sub_sampens = sub_sampens,
  sub_meths   = sub_meths,
  groups      = groups,
  formula     = "sampen ~ meth + I(meth^2) + loc + patient",
...
hist(res3$coefs_df$p_val)
table(res3$coefs_df$adj_p < 0.05)
```
