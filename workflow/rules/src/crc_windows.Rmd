...
head(rownames(shannons_wide))
head(rownames(med_sampens))
...
rownames(range_sampens) <- paste0('w', 1: nrow(range_sampens))
...
tt_sampen <- topTable(fit_sampen, coef = "meta$locationPT", n = Inf, sort.by="none")

##stopifnot(all(rownames(sampen_fd) == rownames(tt_sampen)))
...
```{r}
filepaths <- list.files( snakemake@params[["output_path"]], pattern = "\\.meth\\.out$", full.names = TRUE)

#  (filepaths <- list.files('/home/imallona/src/yamet/workflow/data/crc/windows_output',
#                           pattern = "\\.meth\\.out$", full.names = TRUE))
...
process_meth_reports <- function(filepath) {
    ...
    return(list(meth = list(file = filepath,
                              patient = patient,
                              location = location,
                              meth = se)))
}
...
meths <- lapply(filepaths, function(x) process_meth_reports(x)$meth)


meths_fd <- do.call(cbind, lapply(meths, function(x) x$meth))
...
tt_meth <- topTable(fit_meth, coef = "meta$locationPT", n = Inf, sort.by="none")

##stopifnot(all(rownames(meths_fd) == rownames(tt_meth)))
...
row_tt <- rowAnnotation(limma = as.matrix(data.frame(PT_de_meth = signif_meth,
...
This correction doesn't make much sense to be honest

```{r}

colnames(meths_fd) <- paste0('meth', colnames(meths_fd))
colnames(sampen_fd) <- paste0('sampen', colnames(sampen_fd))

fd <- cbind(meths_fd, sampen_fd)
rm(meths_fd, sampen_fd)

meta <- data.frame(
  colname = colnames(fd),
  patient = sapply(strsplit(colnames(fd), "_"), function(x) x[3]),
  location = substr(sapply(strsplit(colnames(fd), "_"), function(x) x[4]), 1,2),
  score = ifelse(grepl('^meth', colnames(fd)), yes = 'meth', no = 'sampen'))

meta$location <- relevel(factor(meta$location), ref = "NC")
meta$score <- relevel(factor(meta$score), ref = "sampen")


design <- model.matrix(~ patient + location * score, data = meta)
(colnames(design) <- gsub(':', '_', colnames(design)))

fit_both <- lmFit(fd, design)

con <- makeContrasts(locationPT - (scoremeth + locationPT_scoremeth)/2,
                     levels=design)


fit_both <- contrasts.fit(fit_both, con)
fit_both <- eBayes(fit_both)

tt_both <- topTable(fit_both, n = Inf, sort.by="B")


both <- setNames(tt_both$adj.P.Val < 0.01, rownames(tt_both))
#both[both] <- 'corrected_signif'
meth <- setNames(tt_meth$adj.P.Val < 0.01, rownames(tt_meth))
#meth[meth] <- 'meth_signif'
sampen <- setNames(tt_sampen$adj.P.Val < 0.01, rownames(tt_sampen))
#sampen[sampen] <- 'sampen_signif'

meth <- meth[names(both)]
sampen <- sampen[names(both)]

status <- data.frame(both = both,
                     meth = meth,
                     sampen = sampen)

sum(status$both & status$meth & status$sampen, na.rm = TRUE)
sum(status$both & status$meth,na.rm = TRUE)
sum(status$both & status$sampen, na.rm = TRUE)

```


```{r with_limma_2, fig.width = 18, fig.height = 10}

tt_both <- tt_both[rownames(med_sampens),]
signif_both <- ifelse(tt_both$adj.P.Val < 0.05, yes = 1, no = 0)


row_tt <- rowAnnotation(limma = as.matrix(data.frame(PT_de_meth = signif_meth,
                                                     PT_de_sampen = signif_sampen,
                                                     PT_de_sampen_corrected = signif_both)))
