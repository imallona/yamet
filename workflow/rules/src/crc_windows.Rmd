...
column_ha <- HeatmapAnnotation(
  ...
  col = list(
    location = setNames(
      scales::hue_pal()(length(unique(substr(groups$subloc, 1, 2)))),
      sort(unique(substr(groups$subloc, 1, 2)))
    )
  )
)
...
medians come from the `sampens` list already in memory (computed in the QC section).
