...
sampens_list <- lapply(filepaths, \(x) process_reports(x)$sampen)
```
...
saveRDS(windows_sce, file = 'sce_windows_colon.rds')
```
...
row_ha <- rowAnnotation(tile_annotation = as.matrix(data.frame(
  ...
  genes = annot[coefs_df_top$region, "genes_genes"],
  islands = annot[coefs_df_top$region, "cpgIslandExt_cpgIslandExt"],

  pmd = annot[coefs_df_top$region, "pmds_pmd"],
  hmd = annot[coefs_df_top$region, "hmds_pmd"],

  scna_nc = annot[coefs_df_top$region, "crc01_nc_scna_scna"],
  scna_gain = annot[coefs_df_top$region, "crc01_gain_scna_scna"],
  scna_loss = annot[coefs_df_top$region, "crc01_lost_scna_scna"],

  H3K27me3 = annot[coefs_df_top$region, "H3K27me3_chip"],
  H3K9me3 = annot[coefs_df_top$region, "H3K9me3_chip"],
  H3K4me3 = annot[coefs_df_top$region, "H3K4me3_chip"],
  laminb1 = annot[coefs_df_top$region, "laminb1_lad"],

  X0_Enhancer_hmm = annot[coefs_df_top$region, "0_Enhancer_hmm"],
  X2_Enhancer_hmm = annot[coefs_df_top$region, "2_Enhancer_hmm"],
  X1_Transcribed_hmm = annot[coefs_df_top$region, "1_Transcribed_hmm"],
  X4_Transcribed_hmm = annot[coefs_df_top$region, "4_Transcribed_hmm"],
  X5_RegPermissive_hmm = annot[coefs_df_top$region, "5_RegPermissive_hmm"],
  X7_RegPermissive_hmm = annot[coefs_df_top$region, "7_RegPermissive_hmm"],

  X3_Quiescent = annot[coefs_df_top$region, "3_Quiescent_hmm"],
  X8_Quiescent = annot[coefs_df_top$region, "8_Quiescent_hmm"],
  X10_Quiescent = annot[coefs_df_top$region, "10_Quiescent_hmm"],

  X11_Promoter = annot[coefs_df_top$region, "11_Promoter_hmm"],
  X12_Promoter = annot[coefs_df_top$region, "12_Promoter_hmm"],

  X9_ConstitutiveHet = annot[coefs_df_top$region, "9_ConstitutiveHet_hmm"],
  X13_ConstitutiveHet = annot[coefs_df_top$region, "13_ConstitutiveHet_hmm"],

  X6_LowConfidence = annot[coefs_df_top$region, "6_LowConfidence_hmm"]
)))
...
Read meth-normalized Shannon (adjH) from `.norm.det.out.gz` files. Only the summary
column is loaded (per-cell columns are skipped), so this is memory-efficient. adjS
medians come from `sampens_list` already in memory (computed in the QC section).
Per-annotation R2 is then computed for both metrics with location as the predictor.

```{r read_adj_shannons}
norm_fps <- list.files(
  snakemake@params[["output_path"]],
  pattern = paste0(snakemake@wildcards[["win_size"]], "_(.*)\\.norm\\.det\\.out\\.gz$"),
  full.names = TRUE
)

regex_norm <- "(.*)_(.*)_(.*)\\.norm\\.det\\.out\\.gz$"

adj_shannons_long <- bplapply(norm_fps, function(fp) {
  fn  <- basename(fp)
  pat <- gsub(regex_norm, "\\2", fn)
  loc <- gsub(regex_norm, "\\3", fn)
  dt  <- read_tsv(fp,
                  col_select = c(chr, start, end, shannon_norm),
                  na = "-1", n_max = NROWS,
                  show_col_types = FALSE)
  data.frame(
    window   = sprintf("%s:%s-%s", dt$chr, dt$start, dt$end),
    adjH     = dt$shannon_norm,
    patient  = pat,
    location = loc,
...
## use per-window medians already computed in the QC section
...
```{r r2_driver_windows}
## annotation columns: value > 0 means window belongs to that annotation
ann_cols_win <- colnames(annot)
ann_cols_win <- ann_cols_win[sapply(annot, function(x)
  is.numeric(x) && any(!is.na(x) & x > 0))]

common_win <- intersect(adj_shannons_long$window, adj_sampens_long$window)
...
r2_drivers_win <- purrr::compact(lapply(ann_cols_win, function(a) {
  ...
  sub_H <- adj_shannons_long[adj_shannons_long$window %in% members, ]
  sub_S <- adj_sampens_long [adj_sampens_long$window  %in% members, ]

  data.frame(
    annotation = a,
    r2_adjH    = r2_safe_win(sub_H$adjH, sub_H$location),
    r2_adjS    = r2_safe_win(sub_S$adjS, sub_S$location),
    n_windows  = length(members)
  )
})) %>% bind_rows()

r2_drivers_win <- r2_drivers_win %>%
  filter(!is.na(r2_adjH) | !is.na(r2_adjS)) %>%
  mutate(
    r2_adjH = ifelse(is.na(r2_adjH), 0, r2_adjH),
    r2_adjS = ifelse(is.na(r2_adjS), 0, r2_adjS),
    driver = case_when(
      r2_adjH >= 0.3 & r2_adjS <  0.3 ~ "across-cell driven",
      r2_adjS >= 0.3 & r2_adjH <  0.3 ~ "within-cell driven",
...
## R2 scatter: adjH vs adjS

```{r r2_win_scatter, fig.width = 5, fig.height = 5}
ggplot(r2_drivers_win,
  ...
  scale_color_manual(values = driver_pal) +
  scale_shape_manual(values = driver_shapes) +
  scale_x_continuous(limits = c(0, 1)) +
  scale_y_continuous(limits = c(0, 1)) +
  labs(x = "R2: adjH ~ location", y = "R2: adjS ~ location") +
  theme_ng() +
...
ggplot(hm_win, aes(x = adjH, y = adjS, color = location, label = location)) +
  ...
  labs(x = "median adjH", y = "median adjS",
       title = "Global adjH vs adjS per biopsy location") +
  theme_ng() +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
```
