...
# Single cell visualization/ UMAP and SCE generation
...
head(rownames(df_all_filtered))
head(colnames(features))
df_all_filtered$mean_sampen <- rowMeans(features, na.rm = TRUE)
df_all_filtered$var_sampen  <- apply(features, 1, var, na.rm = TRUE)

col_data <- data.frame(df_all_filtered[,c('cell', 'patient', 'location', 'mean_sampen', 'var_sampen')])
head(rownames(col_data))
...
head(colnames(assay(sce, "sampen")))
head(colnames(meth_mat))

meth_mat <- meth_mat[rownames(sce), colnames(assay(sce, "sampen"))]

stopifnot(identical(rownames(assay(sce, "sampen")), rownames(meth_mat)))
stopifnot(identical(colnames(assay(sce, "sampen")), colnames(meth_mat)))
