...
plot(pmds[,grep('by', colnames(pmds), invert = TRUE)],
     pch = ".", col = as.numeric(pmds$by_meth_nc), main = 'by_meth_nc')
```
...
plot(pmds[,grep('by', colnames(pmds), invert = TRUE)],
     pch = ".", col = as.numeric(pmds$by_med_sampen_pt), main = "by_med_sampen_PT")
...
plot(pmds[,grep('by', colnames(pmds), invert = TRUE)],
     pch = ".", col = as.numeric(pmds$by_med_sampen_nc), main = "by_med_sampen_nc")
...
plot(pmds[,grep('by', colnames(pmds), invert = TRUE)],
     pch = ".", col = as.numeric(pmds$by_meth_change), main = "by_meth_change")
```
...
pal <- rainbow(nrow(re), alpha = 0.2)
pal_solid <- rainbow(nrow(re), alpha = 1)
## m1 <- lmer(
...
lapply(seq_len(nrow(re)), \(x) abline(fe[1] + re[x, 1], fe[2] + re[x, 1], col=pal_solid[x]))
...
pmds$meth_nc_rank <- rank(pmds$meth_PT, ties.method = "average")
...
plot(pmds[,c(grep('rank', colnames(pmds), value = TRUE), 'meth_PT')],
     pch = '.', col = as.numeric(pmds$by_meth_pt), main = 'by meth pt')
```
...
plot(pmds[,c(grep('rank', colnames(pmds), value = TRUE), 'med_sampen_PT')],
     pch = '.', col = as.numeric(pmds$by_sampen_pt), main = 'by sampen pt')
...
plot(pmds[,c(grep('rank', colnames(pmds), value = TRUE), 'med_sampen_NC')],
     pch = '.', col = as.numeric(pmds$by_sampen_nc), main = 'by sampen nc')
...
plot(pmds[,c(grep('rank', colnames(pmds), value = TRUE), 'shannon_PT')],
     pch = '.', col = as.numeric(pmds$by_shannon_pt), main = 'by shannon pt')

```

### Ranks associations - by shannon PC

```{r, fig.width = 8, fig.height = 8}

plot(pmds[,c(grep('rank', colnames(pmds), value = TRUE), 'shannon_NC')],
     pch = '.', col = as.numeric(pmds$by_shannon_nc), main = 'by shannon nc')
