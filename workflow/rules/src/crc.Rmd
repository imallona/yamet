...
process_reports <- function(filepath) {
  ...
  m_out <- read_tsv(m_out_path, show_col_types = FALSE, na = "-1")

  se <- as(as.matrix(d_out[, grep("singleC", colnames(d_out))]), "dgCMatrix")
  ms <- as(as.matrix(m_out[, grep("singleC", colnames(m_out))]), "dgCMatrix")
  rownames(se) <- paste0(d_out$chr, ":", d_out$start, "-", d_out$end)
  rownames(ms) <- rownames(se)
...
fit <- lmFit(sampens, design)
fit <- eBayes(fit)
samp_regions <- rownames(topTable(fit, coef = "locPT", number = 20000))
topTable(fit, coef = "locPT")

fit <- lmFit(meths, design)
fit <- eBayes(fit)
meth_regions <- rownames(topTable(fit, coef = "locPT", number = 20000))
topTable(fit, coef = "locPT")
