...
process_reports <- function(filepath) {
  dt <- read_tsv(filepath, show_col_types = FALSE, na = "-1")

  se <- as(as.matrix(dt[, grep("singleC", colnames(dt))]), "dgCMatrix")
  rownames(se) <- paste0(dt$chr, ":", dt$start, "-", dt$end)

  return(list(shannon = dt[, c("shannon", "avg_meth")], sampen = se))
}

param <- MulticoreParam(workers = snakemake@threads)
all_data <- bplapply(filepaths, process_reports, BPPARAM = param)
sampens <- do.call(cbind, lapply(all_data, function(x) x$sampen))

meta <- data.frame(
  colname = colnames(sampens),
  patient = sapply(strsplit(colnames(sampens), "_"), function(x) x[3]),
  subloc = sapply(strsplit(colnames(sampens), "_"), function(x) x[4])
)
...
sub_sampens <- do.call(cbind, lapply(seq_len(nrow(groups)), function(i) {
...
design <- model.matrix(~ meta$patient + meta$loc)
fit <- lmFit(sampens, design)
fit <- eBayes(fit)
regions <- rownames(topTable(fit, coef = "meta$locPT", number = 6000))
