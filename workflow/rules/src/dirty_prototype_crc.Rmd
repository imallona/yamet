...
set.seed(36)
ts <- sample(ncol(tumors), 5)
set.seed(35466)
ns <- sample(ncol(normals), 5)


tumors <- tumors[,grep('yametized', colnames(tumors))[ts]]
normals <- normals[,grep('yametized', colnames(normals))[ns]]
...
Wait, let's order by normal avg_meth status and then browse the shannon's and the per cell sampens.
