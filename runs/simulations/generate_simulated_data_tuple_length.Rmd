...
```{r scores_ONGOING, eval = FALSE}

d <- simulate_boundary(chrom = 'chr19', num_tuples = 10000, average_depth = 10,
                       std_depth = 5, tuple_distance = 20, tuple_length = 4)


## proportion of discordant reads (PDR) 
compute_pdr <- function() {
    stop('todo')
}
...
compute_mhl <- function(d, tuple_length, w = 1) {
    ...
    for (i in 1: tuple_length) {
        ...
        ordered <- grep(target, tuples, value = TRUE)

        disordered <- setdiff(tuples, ordered)

        ## number of methylated i-tuples in current tuple_length-tuple
        ## substrings <- setNames(sapply(regexec(target,
        ##                                       tuples),
        ##                               function(x) attributes(x)$match.length),
        ##                        tuples)
        ## attributes(gregexpr(target, tuples)[[1]])
        ## number of methylated i-tuples in current tuple_length-tuple

        ## nope
        ## substrings <- setNames(sapply(gregexpr(target,
        ##                                        tuples),
        ##                               function(x) length(attributes(x)$match.length)),
        ##                        tuples)
        ## stringr::str_locate_all(pattern = target, string = tuples)
        ## substrings

        sapply(tuples, function(x) substring(x, 1:(nchar(x)-i+1), i:nchar(x)))
