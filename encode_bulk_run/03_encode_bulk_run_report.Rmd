...
for (item in d$entropy_fns) {
    tryCatch({
        ...
        d$entropy[[ssample]] <- read.table(file.path('encode_bulk', item), header = FALSE)
...
for (item in d$meth_fns) {
    tryCatch({
        ssample <- dirname(item)
        d$meth[[ssample]] <- read.table(file.path('encode_bulk', item), header = FALSE)
    }, error = function(x) print(x))
}
...
```{r merging}
d$merged <- list()
for (ssample in dirname(d$entropy_fns)) {
...
for (item in names(d)) {
    ...
    d[[item]]$min_entropy_beta[d[[item]]$beta >= 0.5] <- apply(data.frame(pU = 0,
                                                                          pUM = abs(d[d[[item]]$beta >= 0.5,]$pM - d[d[[item]]$beta >= 0.5,]$pU)/2 ,
                                                                          pMU =  0,
                                                                          pMM = d[d[[item]]$beta >= 0.5,]$pU),
                                                               1, entropy)


    d[[item]]$lower_bound <- apply(d[,c('min_entropy_full', 'min_entropy_beta')], 1, min)
}
```
...
for (item in names(d)) {
    ...
    head(d[d[[item]]$standardized_entropy == 0, c('code', 'beta', 'entropy', 'standardized_entropy')])
...
for (item in names(d)) {
    ...
    p4
    ggsave('standardized_vs_entropy.png', plot = p4, width = 10, height=5)
}
```
...
<!-- for (hmm in hmm_colors) { -->
<!--     freq   = hist(d[d$hmm == hmm, 'beta'], breaks=br, include.lowest=TRUE, plot=FALSE) -->
...
<!-- d$beta_categorical <- apply(d, 1, function(x) { -->
...
<!-- table(d$beta_categorical) -->

<!-- p1 <- ggplot(d[!d$in_boundary,], aes(x = factor(beta_categorical), -->
...
<!-- p2 <- ggplot(d[!d$in_boundary,], aes(x = factor(beta_categorical), y = factor(hmm), -->
...
<!-- p3 -->
<!-- ggsave('heat_categorical.png', plot = p4, width = 10, height=5) -->
